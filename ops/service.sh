...
case "$TARGET" in
  ...
  all)
    ...
    case "$ACTION" in
      restart)
        run_remote "systemctl restart dm-dash-backup.timer dm-dash-monthly-report.timer dm-dash-srd-popular-pages.timer && systemctl status dm-dash-backup.timer --no-pager --full && systemctl status dm-dash-monthly-report.timer --no-pager --full && systemctl status dm-dash-srd-popular-pages.timer --no-pager --full"
        ;;
      status)
        run_remote "systemctl status dm-dash-backup.timer --no-pager --full && systemctl status dm-dash-backup.service --no-pager --full && systemctl status dm-dash-monthly-report.timer --no-pager --full && systemctl status dm-dash-monthly-report.service --no-pager --full && systemctl status dm-dash-srd-popular-pages.timer --no-pager --full && systemctl status dm-dash-srd-popular-pages.service --no-pager --full"
        ;;
      logs)
        run_remote "journalctl -u dm-dash-backup.service -u dm-dash-monthly-report.service -u dm-dash-srd-popular-pages.service --no-pager -n 120"
        ;;
      stop)
        run_remote "systemctl stop dm-dash-backup.timer dm-dash-backup.service dm-dash-monthly-report.timer dm-dash-monthly-report.service dm-dash-srd-popular-pages.timer dm-dash-srd-popular-pages.service && echo 'dm-dash-backup + dm-dash-monthly-report + dm-dash-srd-popular-pages timer/services stopped'"
        ;;
      *)
        echo "Usage: $0 {restart|status|logs|stop} [web|backup|report|top10|all]"
        exit 1
