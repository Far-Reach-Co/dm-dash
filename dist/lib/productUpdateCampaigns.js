...
exports.getProductUpdateCampaigns = getProductUpdateCampaigns;
exports.getProductUpdateCampaignBySlug = getProductUpdateCampaignBySlug;
exports.getLatestProductUpdateCampaign = getLatestProductUpdateCampaign;
exports.renderProductUpdateMessage = renderProductUpdateMessage;
const campaigns = [
    {
        slug: "2026-02-feature-roundup",
        subject: "New in Far Reach Co: pins, library upgrades, and AI SRD search",
        intro: "Here is a quick update on features that recently shipped and what we are actively improving next.",
        recent: [
            {
...
function getProductUpdateCampaigns() {
    return campaigns;
}
function getProductUpdateCampaignBySlug(slug) {
    return campaigns.find((campaign) => campaign.slug === slug);
}
function getLatestProductUpdateCampaign() {
    return campaigns[campaigns.length - 1];
}
function renderProductUpdateMessage(campaign) {
    const recentHtml = campaign.recent
        .map((section) => {
        const linkHtml = section.linkText && section.linkUrl
